var ye=Object.defineProperty;var ke=(a,e,t)=>e in a?ye(a,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):a[e]=t;var _=(a,e,t)=>ke(a,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(n){if(n.ep)return;n.ep=!0;const s=t(n);fetch(n.href,s)}})();class Se{constructor(){this.state={currentScreen:"triage1",results:null,sessionId:null,startTime:null,formData:{},validationErrors:{},screenHistory:[]},this.listeners=new Set,this.initialize()}initialize(){this.state.sessionId=this.generateSessionId(),this.state.startTime=Date.now(),console.log("Store initialized with session:",this.state.sessionId)}generateSessionId(){return"session_"+Date.now()+"_"+Math.random().toString(36).substr(2,9)}subscribe(e){return this.listeners.add(e),()=>this.listeners.delete(e)}notify(){this.listeners.forEach(e=>e(this.state))}getState(){return{...this.state}}setState(e){this.state={...this.state,...e},this.notify()}navigate(e){console.log(`Navigating from ${this.state.currentScreen} to ${e}`);const t=[...this.state.screenHistory];this.state.currentScreen!==e&&!t.includes(this.state.currentScreen)&&t.push(this.state.currentScreen),this.setState({currentScreen:e,screenHistory:t})}goBack(){const e=[...this.state.screenHistory];if(console.log("goBack() - current history:",e),console.log("goBack() - current screen:",this.state.currentScreen),e.length>0){const t=e.pop();return console.log("goBack() - navigating to:",t),this.setState({currentScreen:t,screenHistory:e}),!0}return console.log("goBack() - no history available"),!1}goHome(){this.setState({currentScreen:"triage1",screenHistory:[]})}setFormData(e,t){const i={...this.state.formData};i[e]={...t},this.setState({formData:i})}getFormData(e){return this.state.formData[e]||{}}setValidationErrors(e){this.setState({validationErrors:e})}clearValidationErrors(){this.setState({validationErrors:{}})}setResults(e){this.setState({results:e})}hasUnsavedData(){return Object.keys(this.state.formData).length>0&&!this.state.results}reset(){const e={currentScreen:"triage1",results:null,sessionId:this.generateSessionId(),startTime:Date.now(),formData:{},validationErrors:{},screenHistory:[]};this.setState(e),console.log("Store reset with new session:",e.sessionId)}logEvent(e,t={}){const i={timestamp:Date.now(),session:this.state.sessionId,event:e,data:t};console.log("Event:",i)}getSessionDuration(){return Date.now()-this.state.startTime}}const p=new Se;function A(a){const e=[{id:1,label:"Triage"},{id:2,label:"Assessment"},{id:3,label:"Results"}];let t='<div class="progress-indicator">';return e.forEach((i,n)=>{const s=i.id===a,o=i.id<a;t+=`
  ...
  `}const E={COMA_ICH:"https://europe-west3-igfap-452720.cloudfunctions.net/predict_coma_ich",LDM_ICH:"https://europe-west3-igfap-452720.cloudfunctions.net/predict_limited_data_ich",FULL_STROKE:"https://europe-west3-igfap-452720.cloudfunctions.net/predict_full_stroke"},se={ich:{medium:25,high:50},lvo:{medium:25,high:50}},y={min:29,max:10001},z={autoSaveInterval:18e4,sessionTimeout:30*60*1e3,requestTimeout:1e4},Ee={age_years:{required:!0,min:0,max:120},systolic_bp:{required:!0,min:60,max:300},diastolic_bp:{required:!0,min:30,max:200},gfap_value:{required:!0,min:y.min,max:y.max},fast_ed_score:{required:!0,min:0,max:9}};function Te(){return`
    <div class="container">
      ${A(2)}
  ...
  `;return a!=null&&a.drivers&&(console.log("🧠 Rendering ICH drivers panel"),t+=j(a.drivers,"ICH","ich",a.probability)),e!=null&&e.drivers&&!e.notPossible&&(console.log("🩸 Rendering LVO drivers panel"),t+=j(e.drivers,"LVO","lvo",e.probability)),t+=`
      </div>
    </div>
  `,t}function j(a,e,t,i){if(console.log(`--- ${e} Driver Panel Debug ---`),console.log("Raw drivers input:",a),console.log("Title:",e,"Type:",t,"Probability:",i),!a||Object.keys(a).length===0)return console.log(`No drivers data for ${e}`),`
  ...
  `;return s.length>0?s.forEach(c=>{const u=Math.abs(c.weight*100),g=Math.abs(c.weight)/d*100,b=F(c.label);l+=`
        <div class="compact-driver-item">
          <div class="compact-driver-label">${b}</div>
          <div class="compact-driver-bar positive" style="width: ${g}%">
      ...
      `}):l+=`<div class="no-factors">${r("noPositiveFactors")}</div>`,l+=`
        ...
        <div class="drivers-column negative-column">
          ...
          <div class="compact-drivers">
  `,o.length>0?o.forEach(c=>{const u=Math.abs(c.weight*100),g=Math.abs(c.weight)/d*100,b=F(c.label);l+=`
        <div class="compact-driver-item">
          <div class="compact-driver-label">${b}</div>
          <div class="compact-driver-bar negative" style="width: ${g}%">
            <span class="compact-driver-value">-${u.toFixed(0)}%</span>
          </div>
        </div>
  ...
  `}function _e(a){const e=document.getElementById("useGpsButton"),t=document.getElementById("manualLocationButton"),i=document.querySelector(".location-manual"),n=document.getElementById("locationInput"),s=document.getElementById("searchLocationButton"),o=document.getElementById("strokeCenterResults");e&&e.addEventListener("click",()=>{Be(a,o)}),t&&t.addEventListener("click",()=>{i.style.display=i.style.display==="none"?"block":"none"}),s&&s.addEventListener("click",()=>{const d=n.value.trim();d&&Z(d,a,o)}),n&&n.addEventListener("keypress",d=>{if(d.key==="Enter"){const l=n.value.trim();l&&Z(l,a,o)}})}function Be(a,e){if(!navigator.geolocation){R(r("geolocationNotSupported"),e);return}e.innerHTML=`<div class="loading">${r("gettingLocation")}...</div>`,navigator.geolocation.getCurrentPosition(t=>{const{latitude:i,longitude:n}=t.coords;q(i,n,a,e)},t=>{let i=r("locationError");switch(t.code){case t.PERMISSION_DENIED:i=r("locationPermissionDenied");break;case t.POSITION_UNAVAILABLE:i=r("locationUnavailable");break;case t.TIMEOUT:i=r("locationTimeout");break}R(i,e)},{enableHighAccuracy:!0,timeout:1e4,maximumAge:3e5})}async function Z(a,e,t){t.innerHTML=`<div class="loading">${r("searchingLocation")}...</div>`;const i=/^(-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)$/,n=a.trim().match(i);if(n){const s=parseFloat(n[1]),o=parseFloat(n[2]);if(s>=47.2&&s<=52.5&&o>=5.9&&o<=15){t.innerHTML=`
        <div class="location-success">
          <p>📍 Coordinates: ${s.toFixed(4)}, ${o.toFixed(4)}</p>
        </div>
      `,setTimeout(()=>{q(s,o,e,t)},500);return}else{R("Coordinates appear to be outside Germany. Please check the values.",t);return}}try{let s=a.trim();!s.toLowerCase().includes("deutschland")&&!s.toLowerCase().includes("germany")&&!s.toLowerCase().includes("bayern")&&!s.toLowerCase().includes("bavaria")&&!s.toLowerCase().includes("nordrhein")&&!s.toLowerCase().includes("baden")&&(s=s+", Deutschland");const d=`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(s)}&countrycodes=de&format=json&limit=3&addressdetails=1`,l=await fetch(d,{method:"GET",headers:{Accept:"application/json","User-Agent":"iGFAP-StrokeTriage/2.1.0"}});if(!l.ok)throw new Error(`Geocoding API error: ${l.status}`);const c=await l.json();if(c&&c.length>0){let u=c[0];const g=["Bayern","Baden-Württemberg","Nordrhein-Westfalen"];for(const h of c)if(h.address&&g.includes(h.address.state)){u=h;break}const b=parseFloat(u.lat),v=parseFloat(u.lon),m=u.display_name||a;t.innerHTML=`
        <div class="location-success">
          <p>📍 Found: ${m}</p>
          <small style="color: #666;">Lat: ${b.toFixed(4)}, Lng: ${v.toFixed(4)}</small>
        </div>
      `,setTimeout(()=>{q(b,v,e,t)},1e3)}else R(`
    ...
    `,t)}}async function q(a,e,t,i){var d,l;const n={lat:a,lng:e},s=Ie.routePatient({location:n,ichProbability:((d=t==null?void 0:t.ich)==null?void 0:d.probability)||0,timeFromOnset:(t==null?void 0:t.timeFromOnset)||null,clinicalFactors:(t==null?void 0:t.clinicalFactors)||{}});if(!s||!s.destination){i.innerHTML=`
    ...
    `;return}const o=Fe(s,t);i.innerHTML=`
    <div class="location-info">
      <p><strong>${r("yourLocation")}:</strong> ${a.toFixed(4)}, ${e.toFixed(4)}</p>
      <p><strong>Detected State:</strong> ${Q(s.state)}</p>
    </div>
    <div class="loading">${r("calculatingTravelTimes")}...</div>
  `;try{const c=re[s.state],u=[...c.neurosurgicalCenters,...c.comprehensiveStrokeCenters,...c.regionalStrokeUnits,...c.thrombolysisHospitals||[]],g=s.destination;g.distance=H(a,e,g.coordinates.lat,g.coordinates.lng);try{const m=await J(a,e,g.coordinates.lat,g.coordinates.lng);g.travelTime=m.duration,g.travelSource=m.source}catch{g.travelTime=Math.round(g.distance/.8),g.travelSource="estimated"}const b=u.filter(m=>m.id!==g.id).map(m=>({...m,distance:H(a,e,m.coordinates.lat,m.coordinates.lng)})).sort((m,h)=>m.distance-h.distance).slice(0,3);for(const m of b)try{const h=await J(a,e,m.coordinates.lat,m.coordinates.lng);m.travelTime=h.duration,m.travelSource=h.source}catch{m.travelTime=Math.round(m.distance/.8),m.travelSource="estimated"}const v=`
      <div class="location-info">
        <p><strong>${r("yourLocation")}:</strong> ${a.toFixed(4)}, ${e.toFixed(4)}</p>
        <p><strong>State:</strong> ${Q(s.state)}</p>
        ${o}
      </div>
    ...
    `}}function Q(a){return{bayern:"Bayern (Bavaria)",badenWuerttemberg:"Baden-Württemberg",nordrheinWestfalen:"Nordrhein-Westfalen (NRW)"}[a]||a}function Fe(a,e){var n;const t=Math.round((((n=e==null?void 0:e.ich)==null?void 0:n.probability)||0)*100);let i="🏥";return a.urgency==="IMMEDIATE"?i="🚨":a.urgency==="TIME_CRITICAL"?i="⏰":a.urgency==="URGENT"&&(i="⚠️"),`
  ...
  `}function X(a,e,t){const i=[];a.neurosurgery&&i.push("🧠 Neurosurgery"),a.thrombectomy&&i.push("🩸 Thrombectomy"),a.thrombolysis&&i.push("💉 Thrombolysis");const n=a.network?`<span class="network-badge">${a.network}</span>`:"";return`
  ...
  `}function He(a,e){const t=Number(a),i=se[e];return t>=i.high?"🔴 HIGH RISK":t>=i.medium?"🟡 MEDIUM RISK":"🟢 LOW RISK"}const V={moderate:{min:10},high:{min:20},critical:{min:30}};function le(a){if(!a||a<=0)return{volume:0,volumeRange:{min:0,max:0},riskLevel:"low",mortalityRate:"~0%",isValid:!0,calculation:"No hemorrhage detected"};const e=Math.min(a,1e4);a>1e4&&console.warn(`GFAP value ${a} exceeds expected range, capped at 10,000 pg/ml`);try{const t=.0192+.4533*Math.log10(e),i=Math.pow(10,t),n={min:i*.7,max:i*1.3},s=xe(i),o=Oe(i),d=i<1?"<1":i.toFixed(1);return{volume:i,displayVolume:d,volumeRange:{min:n.min.toFixed(1),max:n.max.toFixed(1)},riskLevel:s,mortalityRate:o,isValid:!0,calculation:`Based on GFAP ${a} pg/ml`,threshold:i>=30?"SURGICAL":i>=20?"HIGH_RISK":"MANAGEABLE"}}catch(t){return console.error("Volume calculation error:",t),{volume:0,volumeRange:{min:0,max:0},riskLevel:"low",mortalityRate:"Unknown",isValid:!1,calculation:"Calculation error",error:t.message}}}function xe(a){return a>=V.critical.min?"critical":a>=V.high.min?"high":a>=V.moderate.min?"moderate":"low"}function Oe(a){return a<10?"5-10%⁴":a<30?`${Math.round(10+(a-10)*9/20)}%⁴`:a<50?`${Math.round(19+(a-30)*25/20)}%³`:a<60?`${Math.round(44+(a-50)*47/10)}%²`:a<80?`${Math.round(91+(a-60)*5/20)}%¹`:"96-100%¹"}function Ve(a){return a<1?"<1 ml":a<10?`${a.toFixed(1)} ml`:`${Math.round(a)} ml`}function ze(a){if(!a||a<=0)return`
    ...
    `;const e=Ve(a),t=`volume-canvas-${Math.random().toString(36).substr(2,9)}`;return`
...
`)}static exportAsJSON(){const e=this.getStoredData();return JSON.stringify(e,null,2)}static downloadData(e="csv"){try{const t=e==="csv"?this.exportAsCSV():this.exportAsJSON(),i=`igfap-research-${Date.now()}.${e}`,n=new Blob([t],{type:e==="csv"?"text/csv":"application/json"}),s=URL.createObjectURL(n),o=document.createElement("a");return o.href=s,o.download=i,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(s),console.log(`📥 Downloaded research data: ${i}`),!0}catch(t){return console.error("Failed to download research data:",t),!1}}static clearData(){try{return localStorage.removeItem(this.STORAGE_KEY),console.log("🗑️ Research data cleared"),!0}catch(e){return console.warn("Failed to clear research data:",e),!1}}static getDataSummary(){var s,o;const e=this.getStoredData();if(!e.entries||e.entries.length===0)return{totalEntries:0,dateRange:null,avgDifference:null};const t=e.entries,i=t.map(d=>{var l,c;return(c=(l=d.comparison)==null?void 0:l.differences)==null?void 0:c.absolute}).filter(d=>d!=null),n=i.length>0?i.reduce((d,l)=>d+Math.abs(l),0)/i.length:0;return{totalEntries:t.length,dateRange:{first:(s=t[0])==null?void 0:s.timestamp,last:(o=t[t.length-1])==null?void 0:o.timestamp},avgAbsoluteDifference:Math.round(n*10)/10,storageSize:JSON.stringify(e).length}}static generateEntryId(){return Date.now().toString(36)+Math.random().toString(36).substr(2)}static getSessionId(){let e=sessionStorage.getItem("research_session_id");return e||(e="session_"+Date.now().toString(36),sessionStorage.setItem("research_session_id",e)),e}}_(C,"STORAGE_KEY","igfap_research_data"),_(C,"MAX_ENTRIES",1e3);function Ue(a,e,t){try{if(!T())return;const i={inputs:{age:t.age_years||t.age,gfap:t.gfap_value||t.gfap,module:a.module||"unknown"},main:{probability:a.probability,module:a.module,confidence:a.confidence},legacy:e,comparison:e?N.compareModels(a,e):null};C.logComparison(i)}catch(i){console.warn("Research logging failed (non-critical):",i)}}function T(a=null){var e;if(a==="coma")return!1;if(a==="limited"||a==="full")return!0;if(typeof window<"u")try{const t=window.store||((e=require("../state/store.js"))==null?void 0:e.store);if(t){const i=t.getState().formData;return i.limited||i.full}}catch{}return!1}function ce(){return""}function de(a,e,t){if(!(e!=null&&e.isValid))return console.log("🔬 Legacy model results invalid:",e),`
    ...
    `;const i=N.compareModels(a,e);return`
    <div class="research-panel" id="researchPanel" style="display: none;">
      <div class="research-header">
        <h4>🔬 Model Comparison (Research)</h4>
        <button class="close-research" id="closeResearch">×</button>
  ...
  `}function Ge(a,e){let t=a.probability||0;t<=1&&(t=t*100);const i=e.probability||0;return`
  ...
  `}function Ye(a){if(!a.isValid)return'<div class="comparison-error">Unable to compare models</div>';const{differences:e,agreement:t}=a;return`
  ...
  `}function Je(){const a=N.getModelMetadata();return`
  ...
  `:""}function K(a,e,t){if(!e)return"";const i=Math.round((e.probability||0)*100),n=He(i,a),s=i>70,o=i>se[a].high,d={ich:"🩸",lvo:"🧠"},l={ich:r("ichProbability"),lvo:r("lvoProbability")},c={ich:"ICH",lvo:$.getCurrentLanguage()==="de"?"Großgefäßverschluss":"Large Vessel Occlusion"};return`
        ...
        <div class="risk-assessment">
          ${a==="ich"&&i>=50&&x()>0?`
            <div class="mortality-assessment">
              ${r("predictedMortality")}: ${le(x()).mortalityRate}
            </div>
          `:""}
        </div>
      </div>
    </div>
  `}function Qe(a){const e=a.gfap_value||x();if(!e||e<=0)return"";const t=le(e);return`
    <div class="volume-display-container">
      ${ze(t.volume)}
    </div>
  `}function x(){var t;const e=p.getState().formData;for(const i of["coma","limited","full"])if((t=e[i])!=null&&t.gfap_value)return parseFloat(e[i].gfap_value);return 0}function Xe(a,e){const{ich:t,lvo:i}=a,n=st(t),s=n!=="coma"?at():null;console.log("🔬 Research Debug - Always Active:",{module:n,researchEnabled:T(n),mainResults:t,legacyResults:s,patientInputs:D(),legacyCalculationAttempted:n!=="coma"}),s&&T(n)&&Ue(t,s,D());const o=(t==null?void 0:t.module)==="Limited"||(t==null?void 0:t.module)==="Coma"||(i==null?void 0:i.notPossible)===!0;t==null||t.module;let d;return o?d=et(t,a,e,s,n):d=tt(t,i,a,e,s,n),setTimeout(()=>{qe()},100),d}function et(a,e,t,i,n){const s=a&&a.probability>.6?ne():"",o=oe(),d=ue(),l=T(n)?ce():"",c=i&&T(n)?de(a,i,D()):"",u=(a==null?void 0:a.module)==="Coma"?it(a.probability):"",g=(a==null?void 0:a.module)!=="Coma"?pe(a.probability):"";return`
    <div class="container">
      ${A(3)}
  ...
  `}function tt(a,e,t,i,n,s){var k,w;const o=Math.round(((a==null?void 0:a.probability)||0)*100);Math.round(((e==null?void 0:e.probability)||0)*100);const d=a&&a.probability>.6?ne():"",l=oe(),c=ue(),u=T(s)?ce():"",g=n&&T(s)?de(a,n,D()):"",m=(((w=(k=p.getState().formData)==null?void 0:k.full)==null?void 0:w.fast_ed_score)||0)>3&&e&&!e.notPossible,h=pe(a.probability);return`
    <div class="container">
      ${A(3)}
      ...
      <div class="${m?"risk-results-dual":"risk-results-single"}">
        ${K("ich",a)}
        ${m?K("lvo",e):""}
      </div>
  ...
  `}function me(a){if(!a||!a.drivers)return'<p class="no-drivers">No driver data available</p>';const e=a.drivers;if(!e.positive&&!e.negative)return'<p class="no-drivers">Driver format error</p>';const t=e.positive||[],i=e.negative||[];return`
    <div class="drivers-split-view">
      <div class="drivers-column positive-column">
        <div class="column-header">
          <span class="column-icon">⬆</span>
        ...
        <div class="compact-drivers">
          ${t.length>0?t.slice(0,5).map(n=>ee(n,"positive")).join(""):`<p class="no-factors">${r("noFactors")||"Keine Faktoren / No factors"}</p>`}
        </div>
      </div>
      ...
      <div class="drivers-column negative-column">
        ...
        <div class="compact-drivers">
          ${i.length>0?i.slice(0,5).map(n=>ee(n,"negative")).join(""):`<p class="no-factors">${r("noFactors")||"Keine Faktoren / No factors"}</p>`}
        </div>
      </div>
    </div>
  `}function ee(a,e){const t=Math.abs(a.weight*100),i=Math.min(t*2,100);return`
    <div class="compact-driver-item">
      <div class="compact-driver-label">${F(a.label)}</div>
      <div class="compact-driver-bar ${e}" style="width: ${i}%;">
        <span class="compact-driver-value">${t.toFixed(1)}%</span>
      </div>
    </div>
  `}function ge(a){if(!a||!a.probability||Math.round((a.probability||0)*100)<50)return"";const t=x();return!t||t<=0?"":`
  ...
  `}function at(a){try{const e=D();if(console.log("🔍 Legacy calculation inputs:",e),!e.age||!e.gfap)return console.warn("🔍 Missing required inputs for legacy model:",{age:e.age,gfap:e.gfap}),null;const t=We(e);return console.log("🔍 Legacy calculation result:",t),t}catch(e){return console.warn("Legacy model calculation failed (non-critical):",e),null}}function D(){const e=p.getState().formData;console.log("🔍 Debug formData structure:",e);let t=null,i=null;for(const s of["coma","limited","full"])e[s]&&(console.log(`🔍 ${s} module data:`,e[s]),t=t||e[s].age_years,i=i||e[s].gfap_value);const n={age:parseInt(t)||null,gfap:parseFloat(i)||null};return console.log("🔍 Extracted patient inputs:",n),n}function pe(a){return Math.round(a*100)>25?`
    ...
    `:""}function it(a){const e=Math.round(a*100),t=$.getCurrentLanguage()==="de";return e>25?`
      <div class="alternative-diagnosis-card">
        <div class="diagnosis-header">
          <span class="lightning-icon">⚡</span>
    ...
    `}function st(a){if(!(a!=null&&a.module))return"unknown";const e=a.module.toLowerCase();return e.includes("coma")?"coma":e.includes("limited")?"limited":e.includes("full")?"full":"unknown"}function nt(a,e,t){const i=[];return t.required&&!e&&e!==0&&i.push("This field is required"),t.min!==void 0&&e!==""&&!isNaN(e)&&parseFloat(e)<t.min&&i.push(`Value must be at least ${t.min}`),t.max!==void 0&&e!==""&&!isNaN(e)&&parseFloat(e)>t.max&&i.push(`Value must be at most ${t.max}`),t.pattern&&!t.pattern.test(e)&&i.push("Invalid format"),i}function rt(a){let e=!0;const t={};return Object.entries(Ee).forEach(([i,n])=>{const s=a.elements[i];if(s){const o=nt(i,s.value,n);o.length>0&&(t[i]=o,e=!1)}}),{isValid:e,validationErrors:t}}function ot(a,e){Object.entries(e).forEach(([t,i])=>{const n=a.querySelector(`[name="${t}"]`);if(n){const s=n.closest(".input-group");if(s){s.classList.add("error"),s.querySelectorAll(".error-message").forEach(d=>d.remove());const o=document.createElement("div");o.className="error-message",o.innerHTML=`<span class="error-icon">⚠️</span> ${i[0]}`,s.appendChild(o)}}})}function lt(a){a.querySelectorAll(".input-group.error").forEach(e=>{e.classList.remove("error"),e.querySelectorAll(".error-message").forEach(t=>t.remove())})}function te(a,e){var o,d;console.log(`=== EXTRACTING ${e.toUpperCase()} DRIVERS ===`),console.log("Full response:",a);let t=null;if(e==="ICH"?(t=((o=a.ich_prediction)==null?void 0:o.drivers)||null,console.log("🧠 ICH raw drivers extracted:",t)):e==="LVO"&&(t=((d=a.lvo_prediction)==null?void 0:d.drivers)||null,console.log("🩸 LVO raw drivers extracted:",t)),!t)return console.log(`❌ No ${e} drivers found`),null;const i=ct(t,e);console.log(`✅ ${e} drivers formatted:`,i);const s=[...i.positive,...i.negative].find(l=>l.label&&(l.label.toLowerCase().includes("fast")||l.label.includes("fast_ed")));return s?console.log(`🎯 FAST-ED found in ${e}:`,`${s.label}: ${s.weight>0?"+":""}${s.weight.toFixed(4)}`):console.log(`⚠️  FAST-ED NOT found in ${e} drivers`),i}function ct(a,e){console.log(`🔄 Formatting ${e} drivers from dictionary:`,a);const t=[],i=[];return Object.entries(a).forEach(([n,s])=>{typeof s=="number"&&(s>0?t.push({label:n,weight:s}):s<0&&i.push({label:n,weight:Math.abs(s)}))}),t.sort((n,s)=>s.weight-n.weight),i.sort((n,s)=>s.weight-n.weight),console.log(`📈 ${e} positive drivers:`,t.slice(0,5)),console.log(`📉 ${e} negative drivers:`,i.slice(0,5)),{kind:"flat_dictionary",units:"logit",positive:t,negative:i,meta:{}}}function ae(a,e){var i,n;console.log(`=== EXTRACTING ${e.toUpperCase()} PROBABILITY ===`);let t=0;return e==="ICH"?(t=((i=a.ich_prediction)==null?void 0:i.probability)||0,console.log("🧠 ICH probability extracted:",t)):e==="LVO"&&(t=((n=a.lvo_prediction)==null?void 0:n.probability)||0,console.log("🩸 LVO probability extracted:",t)),t}function ie(a,e){var i,n;let t=.85;return e==="ICH"?t=((i=a.ich_prediction)==null?void 0:i.confidence)||.85:e==="LVO"&&(t=((n=a.lvo_prediction)==null?void 0:n.confidence)||.85),t}async function dt(){console.log("Warming up Cloud Functions...");const a=Object.values(E).map(async e=>{try{const t=new AbortController,i=setTimeout(()=>t.abort(),3e3);await fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({}),signal:t.signal,mode:"cors"}),clearTimeout(i),console.log(`Warmed up: ${e}`)}catch{console.log(`Warm-up attempt for ${e.split("/").pop()} completed`)}});Promise.all(a).then(()=>{console.log("Cloud Functions warm-up complete")})}class L extends Error{constructor(e,t,i){super(e),this.name="APIError",this.status=t,this.url=i}}function W(a){const e={...a};return Object.keys(e).forEach(t=>{const i=e[t];(typeof i=="boolean"||i==="on"||i==="true"||i==="false")&&(e[t]=i===!0||i==="on"||i==="true"?1:0)}),e}function O(a,e=0){const t=parseFloat(a);return isNaN(t)?e:t}async function U(a,e){const t=new AbortController,i=setTimeout(()=>t.abort(),z.requestTimeout);try{const n=await fetch(a,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(e),signal:t.signal,mode:"cors"});if(clearTimeout(i),!n.ok){let o=`HTTP ${n.status}`;try{const d=await n.json();o=d.error||d.message||o}catch{o=`${o}: ${n.statusText}`}throw new L(o,n.status,a)}return await n.json()}catch(n){throw clearTimeout(i),n.name==="AbortError"?new L("Request timeout - please try again",408,a):n instanceof L?n:new L("Network error - please check your connection and try again",0,a)}}async function ut(a){const e=W(a);console.log("Coma ICH API Payload:",e);try{const t=await U(E.COMA_ICH,e);return console.log("Coma ICH API Response:",t),{probability:O(t.probability||t.ich_probability,0),drivers:t.drivers||null,confidence:O(t.confidence,.75),module:"Coma"}}catch(t){throw console.error("Coma ICH prediction failed:",t),new L(`Failed to get ICH prediction: ${t.message}`,t.status,E.COMA_ICH)}}async function mt(a){const e={age_years:a.age_years,systolic_bp:a.systolic_bp,diastolic_bp:a.diastolic_bp,gfap_value:a.gfap_value,vigilanzminderung:a.vigilanzminderung||0},t=W(e);console.log("Limited Data ICH API Payload:",t);try{const i=await U(E.LDM_ICH,t);return console.log("Limited Data ICH API Response:",i),{probability:O(i.probability||i.ich_probability,0),drivers:i.drivers||null,confidence:O(i.confidence,.65),module:"Limited Data"}}catch(i){throw console.error("Limited Data ICH prediction failed:",i),new L(`Failed to get ICH prediction: ${i.message}`,i.status,E.LDM_ICH)}}async function gt(a){var i,n,s,o,d,l;const e={age_years:a.age_years,systolic_bp:a.systolic_bp,diastolic_bp:a.diastolic_bp,gfap_value:a.gfap_value,fast_ed_score:a.fast_ed_score,headache:a.headache||0,vigilanzminderung:a.vigilanzminderung||0,armparese:a.armparese||0,beinparese:a.beinparese||0,eye_deviation:a.eye_deviation||0,atrial_fibrillation:a.atrial_fibrillation||0,anticoagulated_noak:a.anticoagulated_noak||0,antiplatelets:a.antiplatelets||0},t=W(e);console.log("=== BACKEND DATA MAPPING TEST ==="),console.log("📤 Full Stroke API Payload:",t),console.log("🧪 Clinical Variables Being Sent:"),console.log("  📊 FAST-ED Score:",t.fast_ed_score),console.log("  🩸 Systolic BP:",t.systolic_bp),console.log("  🩸 Diastolic BP:",t.diastolic_bp),console.log("  🧬 GFAP Value:",t.gfap_value),console.log("  👤 Age:",t.age_years),console.log("  🤕 Headache:",t.headache),console.log("  😵 Vigilanz:",t.vigilanzminderung),console.log("  💪 Arm Parese:",t.armparese),console.log("  🦵 Leg Parese:",t.beinparese),console.log("  👁️ Eye Deviation:",t.eye_deviation),console.log("  💓 Atrial Fib:",t.atrial_fibrillation),console.log("  💊 Anticoag NOAK:",t.anticoagulated_noak),console.log("  💊 Antiplatelets:",t.antiplatelets);try{const c=await U(E.FULL_STROKE,t);console.log("📥 Full Stroke API Response:",c),console.log("🔑 Available keys in response:",Object.keys(c)),console.log("=== BACKEND MAPPING VERIFICATION ===");const u=JSON.stringify(c).toLowerCase();console.log("🔍 Backend Feature Name Analysis:"),console.log('  Contains "fast":',u.includes("fast")),console.log('  Contains "ed":',u.includes("ed")),console.log('  Contains "fast_ed":',u.includes("fast_ed")),console.log('  Contains "systolic":',u.includes("systolic")),console.log('  Contains "diastolic":',u.includes("diastolic")),console.log('  Contains "gfap":',u.includes("gfap")),console.log('  Contains "age":',u.includes("age")),console.log('  Contains "headache":',u.includes("headache")),console.log("🧠 ICH driver sources:"),console.log("  response.ich_prediction?.drivers:",(i=c.ich_prediction)==null?void 0:i.drivers),console.log("  response.ich_drivers:",c.ich_drivers),console.log("  response.ich?.drivers:",(n=c.ich)==null?void 0:n.drivers),console.log("  response.drivers?.ich:",(s=c.drivers)==null?void 0:s.ich),console.log("🩸 LVO driver sources:"),console.log("  response.lvo_prediction?.drivers:",(o=c.lvo_prediction)==null?void 0:o.drivers),console.log("  response.lvo_drivers:",c.lvo_drivers),console.log("  response.lvo?.drivers:",(d=c.lvo)==null?void 0:d.drivers),console.log("  response.drivers?.lvo:",(l=c.drivers)==null?void 0:l.lvo),Object.keys(c).forEach(S=>{const f=c[S];typeof f=="number"&&f>=0&&f<=1&&console.log(`Potential probability field: ${S} = ${f}`)});const g=ae(c,"ICH"),b=ae(c,"LVO"),v=te(c,"ICH"),m=te(c,"LVO"),h=ie(c,"ICH"),k=ie(c,"LVO");return console.log("✅ Clean extraction results:"),console.log("  ICH:",{probability:g,hasDrivers:!!v}),console.log("  LVO:",{probability:b,hasDrivers:!!m}),{ich:{probability:g,drivers:v,confidence:h,module:"Full Stroke"},lvo:{probability:b,drivers:m,confidence:k,module:"Full Stroke"}}}catch(c){throw console.error("Full Stroke prediction failed:",c),new L(`Failed to get stroke predictions: ${c.message}`,c.status,E.FULL_STROKE)}}const pt=()=>[{id:"acute_deficit",checked:!1},{id:"symptom_onset",checked:!1},{id:"no_preexisting",checked:!1},{id:"no_trauma",checked:!1}];function ht(){const a=pt();return`
  ...
  `}function bt(){const a=document.getElementById("prerequisitesModal");if(!a){console.error("Prerequisites modal not found");return}console.log("Initializing prerequisites modal");const e=document.getElementById("closePrerequisites"),t=document.getElementById("cancelPrerequisites"),i=document.getElementById("confirmPrerequisites");console.log("Modal buttons found:",{closeBtn:!!e,cancelBtn:!!t,confirmBtn:!!i});const n=()=>{a.remove(),I("welcome")};e==null||e.addEventListener("click",n),t==null||t.addEventListener("click",n),i==null||i.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation(),console.log("Prerequisites confirm button clicked");const d=a.querySelectorAll(".toggle-input"),l=Array.from(d).every(c=>c.checked);if(console.log("All prerequisites checked:",l),l)console.log("Navigating to triage2"),a.remove(),I("triage2");else{console.log("Showing prerequisites warning");const c=document.getElementById("prerequisitesWarning");c&&(c.style.display="flex",c.classList.add("shake"),setTimeout(()=>c.classList.remove("shake"),500))}});const s=a.querySelectorAll(".toggle-input");s.forEach(o=>{o.addEventListener("change",()=>{const d=Array.from(s).every(c=>c.checked),l=document.getElementById("prerequisitesWarning");d&&l&&(l.style.display="none")})})}function vt(){const a=document.getElementById("prerequisitesModal");a&&a.remove();const e=document.createElement("div");e.innerHTML=ht();const t=e.firstElementChild;document.body.appendChild(t),bt()}function ft(a){p.logEvent("triage1_answer",{comatose:a}),a?I("coma"):vt()}function yt(a){p.logEvent("triage2_answer",{examinable:a}),I(a?"full":"limited")}function I(a){p.logEvent("navigate",{from:p.getState().currentScreen,to:a}),p.navigate(a),window.scrollTo(0,0)}function kt(){p.hasUnsavedData()&&!confirm("Are you sure you want to start over? All entered data will be lost.")||(p.logEvent("reset"),p.reset())}function St(){console.log("goBack() called");const a=p.goBack();console.log("goBack() success:",a),a?(p.logEvent("navigate_back"),window.scrollTo(0,0)):(console.log("No history available, going home instead"),he())}function he(){console.log("goHome() called"),p.logEvent("navigate_home"),p.goHome(),window.scrollTo(0,0)}async function wt(a,e){a.preventDefault();const t=a.target,i=t.dataset.module,n=rt(t);if(!n.isValid){ot(e,n.validationErrors);try{const l=Object.keys(n.validationErrors)[0];if(l&&t.elements[l]){const g=t.elements[l];g.focus({preventScroll:!0}),g.scrollIntoView({behavior:"smooth",block:"center"})}const c=document.createElement("div");c.className="sr-only",c.setAttribute("role","status"),c.setAttribute("aria-live","polite");const u=Object.keys(n.validationErrors).length;c.textContent=`${u} field${u===1?"":"s"} need attention.`,document.body.appendChild(c),setTimeout(()=>c.remove(),1200)}catch{}return}const s={};Array.from(t.elements).forEach(l=>{if(l.name)if(l.type==="checkbox")s[l.name]=l.checked;else if(l.type==="number"){const c=parseFloat(l.value);s[l.name]=isNaN(c)?0:c}else l.type==="hidden"&&l.name==="armparese"?s[l.name]=l.value==="true":s[l.name]=l.value}),console.log("Collected form inputs:",s),p.setFormData(i,s);const o=t.querySelector("button[type=submit]"),d=o?o.innerHTML:"";o&&(o.disabled=!0,o.innerHTML=`<span class="loading-spinner"></span> ${r("analyzing")}`);try{let l;switch(i){case"coma":l={ich:await ut(s),lvo:null};break;case"limited":l={ich:await mt(s),lvo:{notPossible:!0}};break;case"full":l=await gt(s);break;default:throw new Error("Unknown module: "+i)}p.setResults(l),p.logEvent("models_complete",{module:i,results:l}),I("results")}catch(l){console.error("Error running models:",l);let c="An error occurred during analysis. Please try again.";l instanceof L&&(c=l.message),Lt(e,c),o&&(o.disabled=!1,o.innerHTML=d)}}function Lt(a,e){a.querySelectorAll(".critical-alert").forEach(n=>{var s,o;(o=(s=n.querySelector("h4"))==null?void 0:s.textContent)!=null&&o.includes("Error")&&n.remove()});const t=document.createElement("div");t.className="critical-alert",t.innerHTML=`<h4><span class="alert-icon">⚠️</span> Error</h4><p>${e}</p>`;const i=a.querySelector(".container");i?i.prepend(t):a.prepend(t),setTimeout(()=>t.remove(),1e4)}function Et(a){const e=document.createElement("div");e.className="sr-only",e.setAttribute("role","status"),e.setAttribute("aria-live","polite");const t={triage1:"Coma assessment",triage2:"Examination capability assessment",coma:"Coma module",limited:"Limited data module",full:"Full stroke assessment",results:"Assessment results"};e.textContent=`Navigated to ${t[a]||a}`,document.body.appendChild(e),setTimeout(()=>e.remove(),1e3)}function Tt(a){const e="iGFAP",i={triage1:"Initial Assessment",triage2:"Examination Capability",coma:"Coma Module",limited:"Limited Data Module",full:"Full Stroke Module",results:"Assessment Results"}[a];document.title=i?`${e} — ${i}`:e}function At(){setTimeout(()=>{const a=document.querySelector("h2");a&&(a.setAttribute("tabindex","-1"),a.focus(),setTimeout(()=>a.removeAttribute("tabindex"),100))},100)}class $t{constructor(){this.scores={facial_palsy:0,arm_weakness:0,speech_changes:0,eye_deviation:0,denial_neglect:0},this.onApply=null,this.modal=null}getTotal(){return Object.values(this.scores).reduce((e,t)=>e+t,0)}getRiskLevel(){return this.getTotal()>=4?"high":"low"}render(){const e=this.getTotal(),t=this.getRiskLevel();return`
