...
console.log(`  GFAP (standardized): ${detailedResult.transformedValues.gfapStandardized.toFixed(4)}`);
console.log(`  FAST-ED (standardized): ${detailedResult.transformedValues.fastedStandardized.toFixed(4)}`);
...
patients.forEach(patient => {
  ...
  console.log(`  GFAP: ${patient.gfap}, FAST-ED: ${patient.fastEd} → ${(prob * 100).toFixed(1)}% (${risk})`);
});
...
console.log(`High GFAP (${highGfapResult.inputs.gfap}): ${(highGfapResult.probability * 100).toFixed(1)}%`);
...
function processStrokeTriageData(payload: StrokeTriagePayload) {
  try {
    ...
    return {
      ...
      risk_level: result.probability > 0.7 ? 'high' :
                 result.probability > 0.4 ? 'moderate' : 'low',
...
function generateClinicalRecommendation(gfap: number, fastEd: number) {
  ...
  return {
    ...
    urgency,
    recommendation,
    fastEdLevel: fastEd >= 10 ? 'Very High' : fastEd >= 6 ? 'High' : fastEd >= 3 ? 'Moderate' : 'Low',
